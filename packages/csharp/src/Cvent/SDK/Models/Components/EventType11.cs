...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class EventType11Extension
    {
        public static string Value(this EventType11 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static EventType11 ToEnum(this string value)
        {
            foreach (var field in typeof(EventType11).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is EventType11)
                    {
                        return (EventType11)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum EventType11");
        }
    }
}