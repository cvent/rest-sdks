...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class ProposalStatusTypeExtension
    {
        public static string Value(this ProposalStatusType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ProposalStatusType ToEnum(this string value)
        {
            foreach (var field in typeof(ProposalStatusType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ProposalStatusType)
                    {
                        return (ProposalStatusType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ProposalStatusType");
        }
    }
}