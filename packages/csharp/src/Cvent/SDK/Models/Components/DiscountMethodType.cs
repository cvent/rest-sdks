...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class DiscountMethodTypeExtension
    {
        public static string Value(this DiscountMethodType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static DiscountMethodType ToEnum(this string value)
        {
            foreach (var field in typeof(DiscountMethodType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is DiscountMethodType)
                    {
                        return (DiscountMethodType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum DiscountMethodType");
        }
    }
}