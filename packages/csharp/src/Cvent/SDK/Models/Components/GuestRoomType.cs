...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class GuestRoomTypeExtension
    {
        public static string Value(this GuestRoomType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static GuestRoomType ToEnum(this string value)
        {
            foreach (var field in typeof(GuestRoomType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is GuestRoomType)
                    {
                        return (GuestRoomType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum GuestRoomType");
        }
    }
}