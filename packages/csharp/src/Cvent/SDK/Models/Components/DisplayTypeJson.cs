...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class DisplayTypeJsonExtension
    {
        public static string Value(this DisplayTypeJson value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static DisplayTypeJson ToEnum(this string value)
        {
            foreach (var field in typeof(DisplayTypeJson).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is DisplayTypeJson)
                    {
                        return (DisplayTypeJson)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum DisplayTypeJson");
        }
    }
}