...
namespace Cvent.SDK.Models.Components
{
    ...
    {
        public static string Value(this QuestionType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static QuestionType ToEnum(this string value)
        {
            foreach (var field in typeof(QuestionType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is QuestionType)
                    {
                        return (QuestionType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum QuestionType");
        }
    }
}