...
namespace Cvent.SDK.Models.Components
{
    ...
    /// </summary>
    public enum AttendeeResponseMethod1
    {
    ...
    public static class AttendeeResponseMethod1Extension
    {
        public static string Value(this AttendeeResponseMethod1 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AttendeeResponseMethod1 ToEnum(this string value)
        {
            foreach (var field in typeof(AttendeeResponseMethod1).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AttendeeResponseMethod1)
                    {
                        return (AttendeeResponseMethod1)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AttendeeResponseMethod1");
        }
    }
}