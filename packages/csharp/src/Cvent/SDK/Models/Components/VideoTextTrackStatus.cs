...
namespace Cvent.SDK.Models.Components
{
    ...
    public enum VideoTextTrackStatus
    {
        ...
        [JsonProperty("Updating")] Updating,
        [JsonProperty("Translating")] Translating,
        [JsonProperty("Transcribing")] Transcribing,
        [JsonProperty("Error")] Error,
    }

    public static class VideoTextTrackStatusExtension
    {
        public static string Value(this VideoTextTrackStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static VideoTextTrackStatus ToEnum(this string value)
        {
            foreach (var field in typeof(VideoTextTrackStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is VideoTextTrackStatus)
                    {
                        return (VideoTextTrackStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum VideoTextTrackStatus");
        }
    }
}