...
#pragma warning disable 0618
    ...
    public static class AttendeeInvitedByExtension
    {
        public static string Value(this AttendeeInvitedBy value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AttendeeInvitedBy ToEnum(this string value)
        {
            foreach (var field in typeof(AttendeeInvitedBy).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AttendeeInvitedBy)
                    {
                        return (AttendeeInvitedBy)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AttendeeInvitedBy");
        }
    }
