...
namespace Cvent.SDK.Models.Components
{
    ...
    public enum AttendeeResponseMethod
    {
...
#pragma warning disable 0618
    ...
    public static class AttendeeResponseMethodExtension
    {
        public static string Value(this AttendeeResponseMethod value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AttendeeResponseMethod ToEnum(this string value)
        {
            foreach (var field in typeof(AttendeeResponseMethod).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AttendeeResponseMethod)
                    {
                        return (AttendeeResponseMethod)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AttendeeResponseMethod");
        }
    }
