...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class OptoutByExtension
    {
        public static string Value(this OptoutBy value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static OptoutBy ToEnum(this string value)
        {
            foreach (var field in typeof(OptoutBy).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is OptoutBy)
                    {
                        return (OptoutBy)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum OptoutBy");
        }
    }
}