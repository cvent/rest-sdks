...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class AttendeeAddStatusExtension
    {
        public static string Value(this AttendeeAddStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AttendeeAddStatus ToEnum(this string value)
        {
            foreach (var field in typeof(AttendeeAddStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AttendeeAddStatus)
                    {
                        return (AttendeeAddStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AttendeeAddStatus");
        }
    }
}