...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class PaymentType12Extension
    {
        public static string Value(this PaymentType12 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static PaymentType12 ToEnum(this string value)
        {
            foreach (var field in typeof(PaymentType12).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is PaymentType12)
                    {
                        return (PaymentType12)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum PaymentType12");
        }
    }
}