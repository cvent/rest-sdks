...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class AnswerType12Extension
    {
        public static string Value(this AnswerType12 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AnswerType12 ToEnum(this string value)
        {
            foreach (var field in typeof(AnswerType12).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AnswerType12)
                    {
                        return (AnswerType12)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AnswerType12");
        }
    }
}