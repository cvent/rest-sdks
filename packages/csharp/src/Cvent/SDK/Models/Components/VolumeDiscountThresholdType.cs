...
namespace Cvent.SDK.Models.Components
{
    ...
    /// </summary>
    public enum VolumeDiscountThresholdType
    {
    ...
    public static class VolumeDiscountThresholdTypeExtension
    {
        public static string Value(this VolumeDiscountThresholdType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static VolumeDiscountThresholdType ToEnum(this string value)
        {
            foreach (var field in typeof(VolumeDiscountThresholdType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is VolumeDiscountThresholdType)
                    {
                        return (VolumeDiscountThresholdType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum VolumeDiscountThresholdType");
        }
    }
}