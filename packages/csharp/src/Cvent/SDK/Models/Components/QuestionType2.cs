...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class QuestionType2Extension
    {
        public static string Value(this QuestionType2 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static QuestionType2 ToEnum(this string value)
        {
            foreach (var field in typeof(QuestionType2).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is QuestionType2)
                    {
                        return (QuestionType2)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum QuestionType2");
        }
    }
}