...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class DateTypeJsonExtension
    {
        public static string Value(this DateTypeJson value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static DateTypeJson ToEnum(this string value)
        {
            foreach (var field in typeof(DateTypeJson).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is DateTypeJson)
                    {
                        return (DateTypeJson)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum DateTypeJson");
        }
    }
}