...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class Region1Extension
    {
        public static string Value(this Region1 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static Region1 ToEnum(this string value)
        {
            foreach (var field in typeof(Region1).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is Region1)
                    {
                        return (Region1)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum Region1");
        }
    }
}