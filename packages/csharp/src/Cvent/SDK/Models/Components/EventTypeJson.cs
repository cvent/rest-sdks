...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class EventTypeJsonExtension
    {
        public static string Value(this EventTypeJson value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static EventTypeJson ToEnum(this string value)
        {
            foreach (var field in typeof(EventTypeJson).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is EventTypeJson)
                    {
                        return (EventTypeJson)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum EventTypeJson");
        }
    }
}