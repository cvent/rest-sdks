...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class ThumbnailStatusExtension
    {
        public static string Value(this ThumbnailStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ThumbnailStatus ToEnum(this string value)
        {
            foreach (var field in typeof(ThumbnailStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is ThumbnailStatus)
                    {
                        return (ThumbnailStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum ThumbnailStatus");
        }
    }
}