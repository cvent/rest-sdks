...
namespace Cvent.SDK.Models.Components
{
    ...
    /// </summary>
    public enum BudgetItemCustomFieldCustomFieldType
    {
    ...
    public static class BudgetItemCustomFieldCustomFieldTypeExtension
    {
        public static string Value(this BudgetItemCustomFieldCustomFieldType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static BudgetItemCustomFieldCustomFieldType ToEnum(this string value)
        {
            foreach (var field in typeof(BudgetItemCustomFieldCustomFieldType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is BudgetItemCustomFieldCustomFieldType)
                    {
                        return (BudgetItemCustomFieldCustomFieldType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum BudgetItemCustomFieldCustomFieldType");
        }
    }
}