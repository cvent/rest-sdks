...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class QuestionType1Extension
    {
        public static string Value(this QuestionType1 value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static QuestionType1 ToEnum(this string value)
        {
            foreach (var field in typeof(QuestionType1).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is QuestionType1)
                    {
                        return (QuestionType1)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum QuestionType1");
        }
    }
}