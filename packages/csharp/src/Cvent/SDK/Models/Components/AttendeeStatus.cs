...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class AttendeeStatusExtension
    {
        public static string Value(this AttendeeStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static AttendeeStatus ToEnum(this string value)
        {
            foreach (var field in typeof(AttendeeStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is AttendeeStatus)
                    {
                        return (AttendeeStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum AttendeeStatus");
        }
    }
}