...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class SupplierTypeExtension
    {
        public static string Value(this SupplierType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static SupplierType ToEnum(this string value)
        {
            foreach (var field in typeof(SupplierType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is SupplierType)
                    {
                        return (SupplierType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum SupplierType");
        }
    }
}