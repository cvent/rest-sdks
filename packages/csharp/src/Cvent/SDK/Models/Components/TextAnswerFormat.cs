...
namespace Cvent.SDK.Models.Components
{
    ...
    public static class TextAnswerFormatExtension
    {
        public static string Value(this TextAnswerFormat value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString()) [0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static TextAnswerFormat ToEnum(this string value)
        {
            foreach (var field in typeof(TextAnswerFormat).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is TextAnswerFormat)
                    {
                        return (TextAnswerFormat)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum TextAnswerFormat");
        }
    }
}