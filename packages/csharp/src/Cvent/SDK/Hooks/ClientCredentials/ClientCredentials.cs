...
namespace Cvent.SDK.Hooks.ClientCredentials
{
    ...
    public class ClientCredentialsHook : ISDKInitHook, IBeforeRequestHook, IAfterErrorHook
    {
        ...
        public ICventSDKHttpClient Client = default!;
        ...
        private Credentials? GetCredentialsGlobal(Func<object> securitySource)
        {
            ...
            if (security?.OAuth2ClientCredentials?.ClientID == null || security?.OAuth2ClientCredentials?.ClientSecret == null || security?.OAuth2ClientCredentials?.TokenURL == null)
            {
                return null;
            }

            return new Credentials(
                security?.OAuth2ClientCredentials?.ClientID!,
                security?.OAuth2ClientCredentials?.ClientSecret!,
                security?.OAuth2ClientCredentials?.TokenURL!,
                security?.OAuth2ClientCredentials?.Scopes
            );
        }
