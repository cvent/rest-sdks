...
/** @internal */
export const PagingJson1$inboundSchema: z.ZodType<
  PagingJson1,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function pagingJson1FromJSON(
  jsonString: string,
): SafeParseResult<PagingJson1, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => PagingJson1$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'PagingJson1' from JSON`,
  );
}
