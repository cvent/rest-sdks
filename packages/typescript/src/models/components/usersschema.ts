...
/** @internal */
export const UsersSchema$inboundSchema: z.ZodType<
  UsersSchema,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function usersSchemaFromJSON(
  jsonString: string,
): SafeParseResult<UsersSchema, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => UsersSchema$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'UsersSchema' from JSON`,
  );
}
