...
/** @internal */
export const AuthenticationSchema$inboundSchema: z.ZodType<
...
export function authenticationSchemaFromJSON(
  jsonString: string,
): SafeParseResult<AuthenticationSchema, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AuthenticationSchema$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AuthenticationSchema' from JSON`,
  );
}
