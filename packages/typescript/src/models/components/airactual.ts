...
export type AirActual = {
   ...
   */
  flightDetail?: Array<FlightDetail> | undefined;
  /**
...
> = z.object({
  ...
  flightDetail: z.array(FlightDetail$inboundSchema).optional(),
  airActualAnswers: z.array(AttendeeAnswer1$inboundSchema).optional(),
});

export function airActualFromJSON(
  jsonString: string,
): SafeParseResult<AirActual, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AirActual$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AirActual' from JSON`,
  );
}
