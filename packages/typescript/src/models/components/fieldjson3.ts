...
/** @internal */
export const FieldJson3$inboundSchema: z.ZodType<
  FieldJson3,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function fieldJson3FromJSON(
  jsonString: string,
): SafeParseResult<FieldJson3, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => FieldJson3$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'FieldJson3' from JSON`,
  );
}
