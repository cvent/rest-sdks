...
import { FieldJson3, FieldJson3$inboundSchema } from "./fieldjson3.js";
import {
  QuestionTypeJson2,
 ...
 */
export type StandardSurveyQuestionJson = {
  /**
   ...
   */
  fields?: Array<FieldJson3> | undefined;
  /**
   * Max possible score
   */
...
/** @internal */
export const StandardSurveyQuestionJson$inboundSchema: z.ZodType<
...
> = z.object({
  ...
  fields: z.array(FieldJson3$inboundSchema).optional(),
...
export function standardSurveyQuestionJsonFromJSON(
  jsonString: string,
): SafeParseResult<StandardSurveyQuestionJson, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => StandardSurveyQuestionJson$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'StandardSurveyQuestionJson' from JSON`,
  );
}
