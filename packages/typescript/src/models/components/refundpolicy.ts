...
export function refundPolicyFromJSON(
  jsonString: string,
): SafeParseResult<RefundPolicy, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => RefundPolicy$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'RefundPolicy' from JSON`,
  );
}
