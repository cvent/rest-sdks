...
import { AdLocation, AdLocation$inboundSchema } from "./adlocation.js";
...
/** @internal */
export const ActivityAd$inboundSchema: z.ZodType<
  ActivityAd,
  z.ZodTypeDef,
  unknown
> = z.object({
  ...
  location: AdLocation$inboundSchema.optional(),
  attribution: z.lazy(() => Attribution$inboundSchema).optional(),
});

export function activityAdFromJSON(
  jsonString: string,
): SafeParseResult<ActivityAd, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ActivityAd$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ActivityAd' from JSON`,
  );
}
