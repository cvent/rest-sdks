 ...
 */
export function budgetUpdateCardTransaction(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateCardTransactionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["card-transaction-update"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/cards/transactions/{transactionId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["budget/transactions:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.CardTransactionCreateResponse$inboundSchema),
