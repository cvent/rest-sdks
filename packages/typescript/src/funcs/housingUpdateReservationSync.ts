...
export function housingUpdateReservationSync(
  ...
  Result<
    components.ExistingReservation,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateReservationSyncRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["new-reservation"], {
    explode: true,
  });

  const pathParams = {
    reservationId: encodeSimple("reservationId", payload.reservationId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/reservations/{reservationId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/reservations:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingReservation$inboundSchema),
