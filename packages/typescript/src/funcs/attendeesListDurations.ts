 ...
 */
export function attendeesListDurations(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListDurationsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListDurationsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/attendance-durations")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendance-durations:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListDurationsResponse$inboundSchema, {
      key: "Result",
    }),
