 ...
 */
export function eventsCopyEvent(
  client: CventSDKCore,
  request: operations.CopyEventRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CopyEventResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CopyEventRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["event-copy"], { explode: true });
  ...
  const path = pathToFunc("/events/{id}/copy")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(202, operations.CopyEventResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
