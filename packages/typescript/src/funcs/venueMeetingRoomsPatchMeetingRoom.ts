 ...
 */
export function venueMeetingRoomsPatchMeetingRoom(
  client: CventSDKCore,
  request: operations.PatchMeetingRoomRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PatchMeetingRoom,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchMeetingRoomRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchMeetingRoom, { explode: true });
  ...
  const path = pathToFunc("/venues/{venueId}/meeting-rooms/{meetingRoomId}")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "patchMeetingRoom",
    oAuth2Scopes: ["venue/meeting-rooms:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PatchMeetingRoom$inboundSchema),
