 ...
 */
export function eventsListMembershipItemsPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListMembershipItemsPostFilterRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/events/{id}/membership-items/filter")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/membership-items:read"],
  ...
  >(
    M.json(
      200,
      operations.ListMembershipItemsPostFilterResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListMembershipItemsPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
