 ...
 */
export function eventsListSessionsEnrollmentPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSessionsEnrollmentPostFilterRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/sessions/enrollment/filter")();
  ...
  const context = {
    ...
    operationID: "listSessionsEnrollmentPostFilter",
    oAuth2Scopes: ["event/session-enrollment:read"],
  ...
  >(
    M.json(
      200,
      operations.ListSessionsEnrollmentPostFilterResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListSessionsEnrollmentPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
