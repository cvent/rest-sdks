 ...
 */
export function audienceSegmentsListSegmentAssociatedAttendees(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSegmentAssociatedAttendeesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    audienceSegmentId: encodeSimple(
      "audienceSegmentId",
  ...
  const path = pathToFunc("/audience-segments/{audienceSegmentId}/attendees")(
    pathParams,
  );
  ...
  const query = encodeFormQuery({
    ...
    "includeDisassociated": payload.includeDisassociated,
    "limit": payload.limit,
    "token": payload.token,
  });
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/audience-segments:read"],
  ...
  >(
    M.json(
      200,
      operations.ListSegmentAssociatedAttendeesResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      audienceSegmentsListSegmentAssociatedAttendees(
        client,
        {
          ...request,
          token: nextCursor,
