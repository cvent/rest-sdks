 ...
 */
export function webcastsListWebcastAttendeeLinks(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListWebcastAttendeeLinksRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/webcasts/{id}/attendee-links")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendee-links:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListWebcastAttendeeLinksResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      webcastsListWebcastAttendeeLinks(
        client,
        {
          ...request,
          token: nextCursor,
