 ...
 */
export function audienceSegmentsListAudienceSegments(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListAudienceSegmentsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAudienceSegmentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/audience-segments")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAudienceSegmentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      audienceSegmentsListAudienceSegments(
        client,
        {
          ...request,
          token: nextCursor,
