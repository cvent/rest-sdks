 ...
 */
export function eventTravelGetHousingReservationRequests(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHousingReservationRequestsRequest$outboundSchema.parse(
        value,
      ),
  ...
  >(
    M.json(
      200,
      operations.GetHousingReservationRequestsResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventTravelGetHousingReservationRequests(
        client,
        {
          ...request,
          token: nextCursor,
