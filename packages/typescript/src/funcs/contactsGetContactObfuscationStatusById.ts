 ...
 */
export function contactsGetContactObfuscationStatusById(
  client: CventSDKCore,
  request: operations.GetContactObfuscationStatusByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ContactObfuscationStatus,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetContactObfuscationStatusByIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    obfuscateId: encodeSimple("obfuscateId", payload.obfuscateId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/contacts/obfuscate/{obfuscateId}/status")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ContactObfuscationStatus$inboundSchema),
