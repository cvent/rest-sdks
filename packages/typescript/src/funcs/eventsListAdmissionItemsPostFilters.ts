 ...
 */
export function eventsListAdmissionItemsPostFilters(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAdmissionItemsPostFiltersRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/admission-items/filter")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/admission-items:read"],
  ...
  >(
    M.json(
      200,
      operations.ListAdmissionItemsPostFiltersResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListAdmissionItemsPostFilters(
        client,
        {
          ...request,
          token: nextCursor,
