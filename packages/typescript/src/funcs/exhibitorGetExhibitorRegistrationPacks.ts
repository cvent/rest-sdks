 ...
 */
export function exhibitorGetExhibitorRegistrationPacks(
  client: CventSDKCore,
  request: operations.GetExhibitorRegistrationPacksRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExhibitorRegistrationPackListResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetExhibitorRegistrationPacksRequest$outboundSchema.parse(
        value,
      ),
  ...
  const context = {
    ...
    operationID: "getExhibitorRegistrationPacks",
    oAuth2Scopes: ["exhibitor/registration-packs:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExhibitorRegistrationPackListResponse$inboundSchema),
