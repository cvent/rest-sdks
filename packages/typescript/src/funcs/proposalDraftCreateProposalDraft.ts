 ...
 */
export function proposalDraftCreateProposalDraft(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.ProposalRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/proposal-drafts")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["proposal/proposals:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateProposalDraftResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
