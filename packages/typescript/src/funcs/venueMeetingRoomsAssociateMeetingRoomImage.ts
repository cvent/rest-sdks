 ...
 */
export function venueMeetingRoomsAssociateMeetingRoomImage(
  client: CventSDKCore,
  request: operations.AssociateMeetingRoomImageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.MeetingRoomImageAssociationResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AssociateMeetingRoomImageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.meetingRoomImageAssociationRequest, {
    explode: true,
  });

  const pathParams = {
    meetingRoomId: encodeSimple("meetingRoomId", payload.meetingRoomId, {
      explode: false,
      charEncoding: "percent",
    }),
    venueId: encodeSimple("venueId", payload.venueId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const context = {
    ...
    operationID: "associateMeetingRoomImage",
    oAuth2Scopes: ["venue/meeting-room-images:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.MeetingRoomImageAssociationResponse$inboundSchema),
