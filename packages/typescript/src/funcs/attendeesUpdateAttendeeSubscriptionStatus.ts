 ...
 */
export function attendeesUpdateAttendeeSubscriptionStatus(
  client: CventSDKCore,
  request: operations.UpdateAttendeeSubscriptionStatusRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttendeeSubscriptionResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAttendeeSubscriptionStatusRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload["attendee-subscription-request"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/attendees/{id}/email-subscriptions")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttendeeSubscriptionResponse$inboundSchema),
