 ...
 */
export function ordersGetAccountOrderItemsByOrderId(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAccountOrderItemsByOrderIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/orders/{orderId}/items")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/orders:read"],
  ...
  >(
    M.json(
      200,
      operations.GetAccountOrderItemsByOrderIdResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      ordersGetAccountOrderItemsByOrderId(
        client,
        {
          ...request,
          token: nextCursor,
