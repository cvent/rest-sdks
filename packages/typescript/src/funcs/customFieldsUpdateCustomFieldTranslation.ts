 ...
 */
export function customFieldsUpdateCustomFieldTranslation(
  client: CventSDKCore,
  request: operations.UpdateCustomFieldTranslationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.Translation1,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateCustomFieldTranslationRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.translation1, { explode: true });
  ...
  const path = pathToFunc("/custom-fields/{customFieldId}/translations")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.Translation1$inboundSchema),
