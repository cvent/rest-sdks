...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateReservationRequestRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["existing-reservation-request"], {
    explode: true,
  });

  const pathParams = {
    reservationRequestsId: encodeSimple(
      "reservationRequestsId",
      payload.reservationRequestsId,
      { explode: false, charEncoding: "percent" },
    ),
  };

  const path = pathToFunc("/reservation-requests/{reservationRequestsId}")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/reservation-requests:write"],
