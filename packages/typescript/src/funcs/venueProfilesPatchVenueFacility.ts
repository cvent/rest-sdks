 ...
 */
export function venueProfilesPatchVenueFacility(
  client: CventSDKCore,
  request: operations.PatchVenueFacilityRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.VenueFacilityPatch,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchVenueFacilityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.venueFacilityPatch, {
    explode: true,
  });
  ...
  const path = pathToFunc("/venues/{venueId}/facility")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["venue/venue-facility:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.VenueFacilityPatch$inboundSchema),
