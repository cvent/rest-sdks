 ...
 */
export function exhibitorListExhibitorCategories(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListExhibitorCategoriesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListExhibitorCategoriesRequest$outboundSchema.parse(value),
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListExhibitorCategoriesResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      exhibitorListExhibitorCategories(
        client,
        {
          ...request,
          token: nextCursor,
