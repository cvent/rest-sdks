 ...
 */
export function eventsListDonationItemsPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListDonationItemsPostFilterRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/donation-items/filter")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/donation-items:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListDonationItemsPostFilterResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListDonationItemsPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
