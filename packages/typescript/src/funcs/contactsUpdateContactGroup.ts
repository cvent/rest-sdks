 ...
 */
export function contactsUpdateContactGroup(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateContactGroupRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["contact-group"], { explode: true });
  ...
  const path = pathToFunc("/contact-groups/{id}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/contact-groups:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ContactGroup$inboundSchema),
