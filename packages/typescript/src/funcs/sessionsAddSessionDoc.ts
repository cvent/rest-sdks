 ...
 */
export function sessionsAddSessionDoc(
  client: CventSDKCore,
  request: operations.AddSessionDocRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SessionFile,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AddSessionDocRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/sessions/{id}/docs/{fileId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SessionFile$inboundSchema),
