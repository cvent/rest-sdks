 ...
 */
export function attendeesUpdateInternalInfoAnswers(
  client: CventSDKCore,
  request: operations.UpdateInternalInfoAnswersRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttendeeInternalInfo,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateInternalInfoAnswersRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["attendee-internal-info"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/attendees/{id}/internal-information-questions")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendees:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttendeeInternalInfo$inboundSchema),
