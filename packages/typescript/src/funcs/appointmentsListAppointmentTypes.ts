 ...
 */
export function appointmentsListAppointmentTypes(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAppointmentTypesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appointment-events/{id}/appointment-types")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["appointments/appointment-types:read"],
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      appointmentsListAppointmentTypes(
        client,
        {
          ...request,
          token: nextCursor,
