 ...
 */
export function badgePrintJobGetBadgePrintJob(
  client: CventSDKCore,
  request: operations.GetBadgePrintJobRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingBadgePrintJob,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetBadgePrintJobRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{eventId}/badge-print-jobs/{jobId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingBadgePrintJob$inboundSchema),
