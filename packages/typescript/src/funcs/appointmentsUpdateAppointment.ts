 ...
 */
export function appointmentsUpdateAppointment(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateAppointmentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["update-appointment-request"], {
    explode: true,
  });

  const pathParams = {
    apptId: encodeSimple("apptId", payload.apptId, {
      explode: false,
  ...
  const path = pathToFunc("/appointment-events/{id}/appointments/{apptId}")(
    pathParams,
  );
