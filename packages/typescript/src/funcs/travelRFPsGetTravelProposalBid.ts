 ...
 */
export function travelRFPsGetTravelProposalBid(
  client: CventSDKCore,
  request: operations.GetTravelProposalBidRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TravelProposalBid2,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTravelProposalBidRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    travelProposalBidId: encodeSimple(
      "travelProposalBidId",
      payload.travelProposalBidId,
      { explode: false, charEncoding: "percent" },
    ),
  };

  const path = pathToFunc("/travel-proposals/bids/{travelProposalBidId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.TravelProposalBid2$inboundSchema),
