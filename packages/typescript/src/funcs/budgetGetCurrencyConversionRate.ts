 ...
 */
export function budgetGetCurrencyConversionRate(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetCurrencyConversionRateRequest$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["budget/currency-conversion-rate:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCurrencyConversionRateResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      budgetGetCurrencyConversionRate(
        client,
        {
          ...request,
          token: nextCursor,
