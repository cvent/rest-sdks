 ...
 */
export function eventsListEventVouchersPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListEventVouchersPostFilterRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/vouchers/filter")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/vouchers:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEventVouchersPostFilterResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListEventVouchersPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
