 ...
 */
export function eventsGetEventPlanningDocuments(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventPlanningDocumentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/planning-documents")(pathParams);
  ...
  const context = {
    ...
    operationID: "getEventPlanningDocuments",
    oAuth2Scopes: ["event/planning-documents:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventPlanningDocumentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsGetEventPlanningDocuments(
        client,
        {
          ...request,
          token: nextCursor,
