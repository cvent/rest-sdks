 ...
 */
export function exhibitorUpdateRegistrationPack(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateRegistrationPackRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["registration-pack-request"], {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    registrationPackId: encodeSimple(
      "registrationPackId",
      payload.registrationPackId,
  ...
  const context = {
    ...
    operationID: "updateRegistrationPack",
    oAuth2Scopes: ["exhibitor/registration-packs:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RegistrationPackResponse$inboundSchema),
