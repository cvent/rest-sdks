 ...
 */
export function attendeesGetBadgesPostFilters(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetBadgesPostFiltersResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetBadgesPostFiltersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/badges/filter")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["exhibitor/badges:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetBadgesPostFiltersResponse$inboundSchema, {
      key: "Result",
    }),
