 ...
 */
export function exhibitorCreateExhibitorCategory(
  client: CventSDKCore,
  request: operations.CreateExhibitorCategoryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExhibitorCategoryResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateExhibitorCategoryRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["exhibitor-category-request"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/events/{id}/exhibitor-categories")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["exhibitor/exhibitor-categories:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ExhibitorCategoryResponse$inboundSchema),
