 ...
 */
export function contactsGetContactIdsByContactGroup(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetContactIdsByContactGroupRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/contact-groups/{id}/contacts")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/contact-groups:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetContactIdsByContactGroupResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      contactsGetContactIdsByContactGroup(
        client,
        {
          ...request,
          token: nextCursor,
