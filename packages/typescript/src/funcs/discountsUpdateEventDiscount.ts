 ...
 */
export function discountsUpdateEventDiscount(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateEventDiscountRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["create-update-discount"], {
    explode: true,
  });

  const pathParams = {
    discountId: encodeSimple("discountId", payload.discountId, {
      explode: false,
  ...
  const path = pathToFunc("/events/{id}/discounts/{discountId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/event-discounts:write"],
