 ...
 */
export function attendeeMessagesGetAttendeeMessagesMembers(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAttendeeMessagesMembersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{eventId}/attendee-messages/members")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendee-messages:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAttendeeMessagesMembersResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      attendeeMessagesGetAttendeeMessagesMembers(
        client,
        {
          ...request,
          token: nextCursor,
