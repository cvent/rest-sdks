 ...
 */
export function appointmentsGetAppointmentEventById(
  client: CventSDKCore,
  request: operations.GetAppointmentEventByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AppointmentEvent,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAppointmentEventByIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appointment-events/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AppointmentEvent$inboundSchema),
