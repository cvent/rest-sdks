 ...
 */
export function seatingGetSeating(
  client: CventSDKCore,
  request: operations.GetSeatingRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingSeating,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSeatingRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/seatings/{seatingId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["seating/event-seatings:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingSeating$inboundSchema),
