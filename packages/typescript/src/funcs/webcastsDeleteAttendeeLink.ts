 ...
 */
export function webcastsDeleteAttendeeLink(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteAttendeeLinkRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    attendeeLinkId: encodeSimple("attendeeLinkId", payload.attendeeLinkId, {
      explode: false,
  ...
  const path = pathToFunc("/webcasts/{id}/attendee-links/{attendeeLinkId}")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendee-links:delete"],
