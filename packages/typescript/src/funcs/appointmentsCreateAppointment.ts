 ...
 */
export function appointmentsCreateAppointment(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateAppointmentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["create-appointment-request"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/appointment-events/{id}/appointments")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["appointments/appointments:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.AppointmentWithQuestions$inboundSchema),
    M.jsonErr(
