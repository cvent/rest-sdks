 ...
 */
export function eventsListQuantityItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListQuantityItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListQuantityItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/quantity-items")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListQuantityItemsResponse$inboundSchema, {
      key: "Result",
    }),
