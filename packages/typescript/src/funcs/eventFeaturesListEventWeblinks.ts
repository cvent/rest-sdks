 ...
 */
export function eventFeaturesListEventWeblinks(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListEventWeblinksResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListEventWeblinksRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/weblinks")(pathParams);
  ...
  const context = {
    ...
    operationID: "listEventWeblinks",
    oAuth2Scopes: ["event/weblinks:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEventWeblinksResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventFeaturesListEventWeblinks(
        client,
        {
          ...request,
          token: nextCursor,
