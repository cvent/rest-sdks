 ...
 */
export function videoUpdateTextTrack(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateTextTrackRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["existing-video-text-track"], {
    explode: true,
  });

  const pathParams = {
    textTrackId: encodeSimple("textTrackId", payload.textTrackId, {
      explode: false,
  ...
  const path = pathToFunc("/videos/{videoId}/text-tracks/{textTrackId}")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/videos:write"],
