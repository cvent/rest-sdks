 ...
 */
export function eventsUpdateQuantityItemRegistrationForAttendee(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateQuantityItemRegistrationForAttendeeRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON("body", payload["quantity-item-update"], {
    explode: true,
  });

  const pathParams = {
    quantityItemId: encodeSimple("quantityItemId", payload.quantityItemId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/quantity-items/{quantityItemId}")(pathParams);
  ...
  const context = {
    ...
    operationID: "updateQuantityItemRegistrationForAttendee",
    oAuth2Scopes: ["event/quantity-items:write"],
