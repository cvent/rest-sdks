 ...
 */
export function sessionsAddSessionLocation(
  client: CventSDKCore,
  request: operations.AddSessionLocationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SessionLocation,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AddSessionLocationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["session-location"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/events/{id}/session-locations")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SessionLocation$inboundSchema),
