 ...
 */
export function eventsGetEventsPostFilters(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetEventsPostFiltersResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventsPostFiltersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/filter")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/events:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventsPostFiltersResponse$inboundSchema, {
      key: "Result",
    }),
