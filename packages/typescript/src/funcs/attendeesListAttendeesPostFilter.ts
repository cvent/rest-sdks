 ...
 */
export function attendeesListAttendeesPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAttendeesPostFilterRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/attendees/filter")();

  const query = encodeFormQuery({
    "after": payload.after,
    "before": payload.before,
    "expand": payload.expand,
    "includeDeletedGuests": payload.includeDeletedGuests,
    "limit": payload.limit,
    "sort": payload.sort,
    "token": payload.token,
  });
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendees:read"],
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      attendeesListAttendeesPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
