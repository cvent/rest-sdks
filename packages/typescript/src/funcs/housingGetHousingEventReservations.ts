...
export function housingGetHousingEventReservations(
  ...
  Result<
    components.ReservationsPaginatedResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHousingEventReservationsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/housing-events/{housingEventId}/reservations")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/reservations:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ReservationsPaginatedResponse$inboundSchema),
