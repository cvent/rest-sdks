 ...
 */
export function speakersAssignSpeakerProfileImage(
  client: CventSDKCore,
  request: operations.AssignSpeakerProfileImageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SpeakerFile,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AssignSpeakerProfileImageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["speaker-file"], { explode: true });
  ...
  const path = pathToFunc("/speakers/{id}/profile-images")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/speakers:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SpeakerFile$inboundSchema),
