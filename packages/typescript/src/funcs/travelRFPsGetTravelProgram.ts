 ...
 */
export function travelRFPsGetTravelProgram(
  client: CventSDKCore,
  request: operations.GetTravelProgramRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TravelProgram,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetTravelProgramRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/travel-programs/{programId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.TravelProgram$inboundSchema),
