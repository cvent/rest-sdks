 ...
 */
export function sessionsCreateSessionCategory(
  client: CventSDKCore,
  request: components.SessionCategoryInput,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SessionCategory,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.SessionCategoryInput$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/session-categories:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SessionCategory$inboundSchema),
