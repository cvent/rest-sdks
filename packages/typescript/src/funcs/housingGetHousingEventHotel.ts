...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHousingEventHotelRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    hotelId: encodeSimple("hotelId", payload.hotelId, {
      explode: false,
      charEncoding: "percent",
    }),
    housingEventId: encodeSimple("housingEventId", payload.housingEventId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/housing-events/{housingEventId}/hotels/{hotelId}")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/housing-event-hotels:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.HousingEventHotel$inboundSchema),
