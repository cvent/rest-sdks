 ...
 */
export function eventsEventCheckIn(
  client: CventSDKCore,
  request: operations.EventCheckInRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    Array<components.AttendeeAddBulkItemEventJson>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.EventCheckInRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/check-in")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(207, z.array(components.AttendeeAddBulkItemEventJson$inboundSchema)),
