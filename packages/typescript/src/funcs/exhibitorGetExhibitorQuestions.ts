 ...
 */
export function exhibitorGetExhibitorQuestions(
  client: CventSDKCore,
  request: operations.GetExhibitorQuestionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExhibitorQuestionsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetExhibitorQuestionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/exhibitor-questions")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["exhibitor/exhibitor-questions:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExhibitorQuestionsResponse$inboundSchema),
