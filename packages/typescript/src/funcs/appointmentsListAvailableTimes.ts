 ...
 */
export function appointmentsListAvailableTimes(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListAvailableTimesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListAvailableTimesRequest$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["appointments/available-times:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAvailableTimesResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      appointmentsListAvailableTimes(
        client,
        {
          ...request,
          token: nextCursor,
