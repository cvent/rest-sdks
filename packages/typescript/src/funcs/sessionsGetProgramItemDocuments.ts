 ...
 */
export function sessionsGetProgramItemDocuments(
  client: CventSDKCore,
  request: operations.GetProgramItemDocumentsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ProgramItemSessionDocuments,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetProgramItemDocumentsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    programItemId: encodeSimple("programItemId", payload.programItemId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/program-items/{programItemId}/docs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ProgramItemSessionDocuments$inboundSchema),
