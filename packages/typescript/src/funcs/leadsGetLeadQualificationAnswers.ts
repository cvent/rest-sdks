 ...
 */
export function leadsGetLeadQualificationAnswers(
  client: CventSDKCore,
  request: operations.GetLeadQualificationAnswersRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.LeadQualificationAnswersListResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetLeadQualificationAnswersRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    leadId: encodeSimple("leadId", payload.leadId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const context = {
    ...
    oAuth2Scopes: ["exhibitor/lead-qualification-answers:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.LeadQualificationAnswersListResponse$inboundSchema),
