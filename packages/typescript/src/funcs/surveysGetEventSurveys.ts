 ...
 */
export function surveysGetEventSurveys(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetEventSurveysResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetEventSurveysRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/surveys")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["survey/surveys:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventSurveysResponse$inboundSchema, {
      key: "Result",
    }),
