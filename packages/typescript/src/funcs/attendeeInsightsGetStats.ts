 ...
 */
export function attendeeInsightsGetStats(
  client: CventSDKCore,
  request: operations.GetStatsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttendeeInsightsStats,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetStatsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/attendee-insights/{id}/stats")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["attendee-insights/stats:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttendeeInsightsStats$inboundSchema),
