 ...
 */
export function eventsPlusHubGetHubMembers(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetHubMembersResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetHubMembersRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    hubId: encodeSimple("hubId", payload.hubId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/hubs/{hubId}/members")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["events-plus/hubs:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetHubMembersResponse$inboundSchema, {
      key: "Result",
    }),
