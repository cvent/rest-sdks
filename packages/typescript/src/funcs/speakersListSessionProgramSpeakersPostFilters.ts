 ...
 */
export function speakersListSessionProgramSpeakersPostFilters(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSessionProgramSpeakersPostFiltersRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/program-items/speakers/filter")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/program-items:read"],
  ...
  >(
    M.json(
      200,
      operations.ListSessionProgramSpeakersPostFiltersResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      speakersListSessionProgramSpeakersPostFilters(
        client,
        {
          ...request,
          token: nextCursor,
