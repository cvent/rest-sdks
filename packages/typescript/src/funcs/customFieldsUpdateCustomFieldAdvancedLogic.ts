 ...
 */
export function customFieldsUpdateCustomFieldAdvancedLogic(
  client: CventSDKCore,
  request: operations.UpdateCustomFieldAdvancedLogicRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AdvancedLogic,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateCustomFieldAdvancedLogicRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload["advanced-logic"], { explode: true });
  ...
  const path = pathToFunc("/custom-fields/{customFieldId}/advanced-logic")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/custom-fields:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AdvancedLogic$inboundSchema),
