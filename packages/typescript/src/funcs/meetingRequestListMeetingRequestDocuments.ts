 ...
 */
export function meetingRequestListMeetingRequestDocuments(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListMeetingRequestDocumentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/meeting-requests/{requestId}/documents")(
    pathParams,
  );

  const query = encodeFormQuery({
    "filter": payload.filter,
    "includeReferenceLink": payload.includeReferenceLink,
    "limit": payload.limit,
    "token": payload.token,
  });
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/meeting-requests:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListMeetingRequestDocumentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      meetingRequestListMeetingRequestDocuments(
        client,
        {
          ...request,
          token: nextCursor,
