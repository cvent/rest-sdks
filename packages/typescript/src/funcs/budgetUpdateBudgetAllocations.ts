 ...
 */
export function budgetUpdateBudgetAllocations(
  client: CventSDKCore,
  request: operations.UpdateBudgetAllocationsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.BudgetAllocationsList,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateBudgetAllocationsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["budget-allocations-list"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.BudgetAllocationsList$inboundSchema),
