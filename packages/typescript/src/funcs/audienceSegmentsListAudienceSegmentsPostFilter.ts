 ...
 */
export function audienceSegmentsListAudienceSegmentsPostFilter(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAudienceSegmentsPostFilterRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/audience-segments/filter")();
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      audienceSegmentsListAudienceSegmentsPostFilter(
        client,
        {
          ...request,
          token: nextCursor,
