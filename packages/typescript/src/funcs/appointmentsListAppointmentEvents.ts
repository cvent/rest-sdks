 ...
 */
export function appointmentsListAppointmentEvents(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListAppointmentEventsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAppointmentEventsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appointment-events")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["appointments/appointment-events:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAppointmentEventsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      appointmentsListAppointmentEvents(
        client,
        {
          ...request,
          token: nextCursor,
