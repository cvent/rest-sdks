 ...
 */
export function badgePrinterPoolsGetBadgePrinterPool(
  client: CventSDKCore,
  request: operations.GetBadgePrinterPoolRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingBadgePrinterPool,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetBadgePrinterPoolRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{eventId}/badge-printer-pools/{poolId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingBadgePrinterPool$inboundSchema),
