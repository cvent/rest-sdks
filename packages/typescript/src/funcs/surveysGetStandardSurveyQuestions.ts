 ...
 */
export function surveysGetStandardSurveyQuestions(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetStandardSurveyQuestionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/standard-surveys/{surveyId}/questions")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["survey/standard-survey-questions:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetStandardSurveyQuestionsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      surveysGetStandardSurveyQuestions(
        client,
        {
          ...request,
          token: nextCursor,
