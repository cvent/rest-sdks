 ...
 */
export function attendeeInsightsGetAttendeeInsightsById(
  client: CventSDKCore,
  request: operations.GetAttendeeInsightsByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttendeeInsights,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAttendeeInsightsByIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/attendee-insights/{id}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["attendee-insights/attendee-insights:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttendeeInsights$inboundSchema),
