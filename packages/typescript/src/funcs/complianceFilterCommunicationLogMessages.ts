 ...
 */
export function complianceFilterCommunicationLogMessages(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.FilterCommunicationLogMessagesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/logs/communications/messages/filter")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["compliance/communications:read"],
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      complianceFilterCommunicationLogMessages(
        client,
        {
          ...request,
          token: nextCursor,
