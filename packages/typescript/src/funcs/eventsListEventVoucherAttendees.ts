 ...
 */
export function eventsListEventVoucherAttendees(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListEventVoucherAttendeesRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    voucherId: encodeSimple("voucherId", payload.voucherId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/events/{id}/vouchers/{voucherId}/attendees")(
    pathParams,
  );
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEventVoucherAttendeesResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      eventsListEventVoucherAttendees(
        client,
        {
          ...request,
          token: nextCursor,
