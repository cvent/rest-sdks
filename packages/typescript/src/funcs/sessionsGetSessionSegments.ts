 ...
 */
export function sessionsGetSessionSegments(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetSessionSegmentsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSessionSegmentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/session-segments")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/session-segments:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetSessionSegmentsResponse$inboundSchema, {
      key: "Result",
    }),
