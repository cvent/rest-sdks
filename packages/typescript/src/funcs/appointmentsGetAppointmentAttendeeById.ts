 ...
 */
export function appointmentsGetAppointmentAttendeeById(
  client: CventSDKCore,
  request: operations.GetAppointmentAttendeeByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AppointmentAttendee,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAppointmentAttendeeByIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appointment-attendees/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AppointmentAttendee$inboundSchema),
