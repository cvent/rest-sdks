 ...
 */
export function eventsListEventEmails(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListEventEmailsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListEventEmailsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/emails")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/event-emails:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEventEmailsResponse$inboundSchema, {
      key: "Result",
    }),
