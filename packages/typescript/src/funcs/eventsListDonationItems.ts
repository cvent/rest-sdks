 ...
 */
export function eventsListDonationItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListDonationItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListDonationItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/donation-items")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListDonationItemsResponse$inboundSchema, {
      key: "Result",
    }),
