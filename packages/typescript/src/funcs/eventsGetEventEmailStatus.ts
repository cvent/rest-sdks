 ...
 */
export function eventsGetEventEmailStatus(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventEmailStatusRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/emails/{emailRequestId}/status")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/event-email-status:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SendEmailEventResponse$inboundSchema),
