 ...
 */
export function emailsGetEmailStatus(
  client: CventSDKCore,
  request: operations.GetEmailStatusRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.EmailSendStatus,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetEmailStatusRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/emails/{emailRequestId}/status")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["email/email-status:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.EmailSendStatus$inboundSchema),
