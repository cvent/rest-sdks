 ...
 */
export function eventsGetOrderItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetOrderItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetOrderItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/orders/items")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetOrderItemsResponse$inboundSchema, {
      key: "Result",
    }),
