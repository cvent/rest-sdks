 ...
 */
export function eventsListFeeItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListFeeItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListFeeItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/fee-items")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/fee-items:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListFeeItemsResponse$inboundSchema, {
      key: "Result",
    }),
