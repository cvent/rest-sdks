 ...
 */
export function campaignsGetEmarketingEmailStatus(
  client: CventSDKCore,
  request: operations.GetEmarketingEmailStatusRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SendEmailEmarketingResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEmarketingEmailStatusRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    emailRequestId: encodeSimple("emailRequestId", payload.emailRequestId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/emarketing/emails/{emailRequestId}/status")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["emarketing/emarketing-email-status:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SendEmailEmarketingResponse$inboundSchema),
