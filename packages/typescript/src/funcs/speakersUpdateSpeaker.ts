 ...
 */
export function speakersUpdateSpeaker(
  client: CventSDKCore,
  request: operations.UpdateSpeakerRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingSpeaker,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateSpeakerRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["existing-speaker"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/speakers/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingSpeaker$inboundSchema),
