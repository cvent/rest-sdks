 ...
 */
export function rfpSuppliersListRFPRecipientsHistory(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListRfpRecipientsHistoryRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rfps/{rfpId}/recipients/history")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["rfp/rfp-recipients-history:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListRfpRecipientsHistoryResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      rfpSuppliersListRFPRecipientsHistory(
        client,
        {
          ...request,
          token: nextCursor,
