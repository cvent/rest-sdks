 ...
 */
export function eventsListRegistrationPaths(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListRegistrationPathsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListRegistrationPathsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/registration-paths")(pathParams);
  ...
  const context = {
    ...
    operationID: "listRegistrationPaths",
    oAuth2Scopes: ["event/registration-paths:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListRegistrationPathsResponse$inboundSchema, {
      key: "Result",
    }),
