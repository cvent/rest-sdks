 ...
 */
export function rfpRequirementsListRFPQuestions(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListRfpQuestionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rfps/{rfpId}/questions")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["rfp/rfp-questions:read"],
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      rfpRequirementsListRFPQuestions(
        client,
        {
          ...request,
          token: nextCursor,
