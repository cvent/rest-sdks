 ...
 */
export function audienceSegmentsListAttendeeAudienceSegments(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAttendeeAudienceSegmentsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAttendeeAudienceSegmentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      audienceSegmentsListAttendeeAudienceSegments(
        client,
        {
          ...request,
          token: nextCursor,
