 ...
 */
export function speakersAddSpeakerCategory(
  client: CventSDKCore,
  request: components.SpeakerCategory2,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingSpeakerCategory,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.SpeakerCategory2$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/speaker-categories:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ExistingSpeakerCategory$inboundSchema),
