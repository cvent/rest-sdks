 ...
 */
export function rfpManagementGetRFPLeadSource(
  client: CventSDKCore,
  request: operations.GetRfpLeadSourceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.RfpLeadSource,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetRfpLeadSourceRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    leadSourceId: encodeSimple("leadSourceId", payload.leadSourceId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/rfps/lead-sources/{leadSourceId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["rfp/rfp-lead-sources:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RfpLeadSource$inboundSchema),
