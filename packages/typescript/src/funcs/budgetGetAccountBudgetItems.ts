 ...
 */
export function budgetGetAccountBudgetItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetAccountBudgetItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAccountBudgetItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/budget-items")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAccountBudgetItemsResponse$inboundSchema, {
      key: "Result",
    }),
