 ...
 */
export function transactionsGetAccountTransactionItems(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAccountTransactionItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/transactions/{transactionId}/items")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/transactions:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAccountTransactionItemsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      transactionsGetAccountTransactionItems(
        client,
        {
          ...request,
          token: nextCursor,
