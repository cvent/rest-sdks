...
export function rfpRequirementsListRFPAgendaItems(
  ...
  PageIterator<
    Result<
      operations.ListRfpAgendaItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListRfpAgendaItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rfps/{rfpId}/agenda-items")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListRfpAgendaItemsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      rfpRequirementsListRFPAgendaItems(
        client,
        {
          ...request,
          token: nextCursor,
