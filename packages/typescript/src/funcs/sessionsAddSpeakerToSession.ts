 ...
 */
export function sessionsAddSpeakerToSession(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AddSpeakerToSessionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["session-speaker-assignment"], {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    speakerId: encodeSimple("speakerId", payload.speakerId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/sessions/{id}/speakers/{speakerId}")(pathParams);
  ...
  const context = {
    ...
    operationID: "addSpeakerToSession",
    oAuth2Scopes: ["event/sessions:write"],
