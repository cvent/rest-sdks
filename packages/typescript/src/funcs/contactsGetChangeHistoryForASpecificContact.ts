 ...
 */
export function contactsGetChangeHistoryForASpecificContact(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetChangeHistoryForASpecificContactRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/contacts/{contactId}/history")(pathParams);
  ...
  >(
    M.json(
      200,
      operations.GetChangeHistoryForASpecificContactResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      contactsGetChangeHistoryForASpecificContact(
        client,
        {
          ...request,
          token: nextCursor,
