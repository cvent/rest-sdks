 ...
 */
export function sessionsListProgramItemsPostFilters(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListProgramItemsPostFiltersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/program-items/filter")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListProgramItemsPostFiltersResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      sessionsListProgramItemsPostFilters(
        client,
        {
          ...request,
          token: nextCursor,
