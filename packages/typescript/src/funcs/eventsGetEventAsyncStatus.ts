 ...
 */
export function eventsGetEventAsyncStatus(
  client: CventSDKCore,
  request: operations.GetEventAsyncStatusRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.EventAsyncStatus,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventAsyncStatusRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    eventCreationId: encodeSimple("eventCreationId", payload.eventCreationId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/events/async/{eventCreationId}/status")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/events:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.EventAsyncStatus$inboundSchema),
