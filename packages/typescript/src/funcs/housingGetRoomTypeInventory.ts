...
export function housingGetRoomTypeInventory(
  ...
  Result<
    components.InventoryDetailsList,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetRoomTypeInventoryRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    roomTypeId: encodeSimple("roomTypeId", payload.roomTypeId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/housing-event-inventory:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InventoryDetailsList$inboundSchema),
