 ...
 */
export function seatingGetEventTableAssignments(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventTableAssignmentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/seatings/assignments")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["seating/assignments:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventTableAssignmentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      seatingGetEventTableAssignments(
        client,
        {
          ...request,
          token: nextCursor,
