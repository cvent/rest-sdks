 ...
 */
export function eventFeaturesUpdateEventFeatures(
  client: CventSDKCore,
  request: operations.UpdateEventFeaturesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.EventFeaturesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateEventFeaturesRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["feature-update"], { explode: true });
  ...
  const path = pathToFunc("/events/{id}/features/{type}")(pathParams);
  ...
  const context = {
    ...
    operationID: "updateEventFeatures",
    oAuth2Scopes: ["event/event-features:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.EventFeaturesResponse$inboundSchema),
