 ...
 */
export function audienceSegmentsListAssociatedAudienceSegments(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAssociatedAudienceSegmentsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    attendeeId: encodeSimple("attendeeId", payload.attendeeId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/attendees/{attendeeId}/audience-segments/filter")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.ListAssociatedAudienceSegmentsResponse$inboundSchema,
      { key: "Result" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      audienceSegmentsListAssociatedAudienceSegments(
        client,
        {
          ...request,
          token: nextCursor,
