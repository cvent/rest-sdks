 ...
 */
export function budgetGetEventBudgetTotals(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetEventBudgetTotalsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventBudgetTotalsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/budget-totals")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["budget/budget-totals:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventBudgetTotalsResponse$inboundSchema, {
      key: "Result",
    }),
