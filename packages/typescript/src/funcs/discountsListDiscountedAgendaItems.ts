 ...
 */
export function discountsListDiscountedAgendaItems(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListDiscountedAgendaItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/discounts/agenda-items")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/event-discounts:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListDiscountedAgendaItemsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      discountsListDiscountedAgendaItems(
        client,
        {
          ...request,
          token: nextCursor,
