 ...
 */
export function surveysGetEventSurveyQuestions(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetEventSurveyQuestionsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetEventSurveyQuestionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/surveys/{surveyId}/questions")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["survey/survey-questions:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetEventSurveyQuestionsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      surveysGetEventSurveyQuestions(
        client,
        {
          ...request,
          token: nextCursor,
