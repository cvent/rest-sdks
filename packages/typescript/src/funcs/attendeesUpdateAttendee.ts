 ...
 */
export function attendeesUpdateAttendee(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateAttendeeRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["attendee-update"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/attendees/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.Attendee$inboundSchema),
