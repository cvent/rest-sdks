 ...
 */
export function audienceSegmentsUpdateAudienceSegment(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAudienceSegmentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["audience-segment"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/audience-segments/{audienceSegmentId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingAudienceSegment$inboundSchema),
    M.jsonErr(
