 ...
 */
export function speakersGetSessionProgramSpeakers(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetSessionProgramSpeakersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/program-items/speakers")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetSessionProgramSpeakersResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      speakersGetSessionProgramSpeakers(
        client,
        {
          ...request,
          token: nextCursor,
