 ...
 */
export function attendeeActivitiesCreateAttendeeActivity(
  client: CventSDKCore,
  request: components.ExternalActivityInput,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExternalActivity,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ExternalActivityInput$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendee-activities:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ExternalActivity$inboundSchema),
