 ...
 */
export function contactsListContactTypes(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListContactTypesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListContactTypesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/contact-types")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/contact-types:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListContactTypesResponse$inboundSchema, {
      key: "Result",
    }),
