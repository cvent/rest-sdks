 ...
 */
export function hooksUpdateContactHook(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateContactHookRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.hook, { explode: true });
  ...
  const path = pathToFunc("/contacts/hooks/{id}")(pathParams);
  ...
  const context = {
    ...
    operationID: "updateContactHook",
    oAuth2Scopes: ["account/hooks:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.HookOutput$inboundSchema),
