 ...
 */
export function appointmentsListAppointmentAttendees(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAppointmentAttendeesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appointment-attendees")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAppointmentAttendeesResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      appointmentsListAppointmentAttendees(
        client,
        {
          ...request,
          token: nextCursor,
