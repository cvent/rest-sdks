 ...
 */
export function contactsAssignContactProfileImage(
  client: CventSDKCore,
  request: operations.AssignContactProfileImageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ContactFile,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AssignContactProfileImageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["contact-file"], { explode: true });
  ...
  const path = pathToFunc("/contacts/{id}/profile-images")(pathParams);
  ...
  const context = {
    ...
    operationID: "assignContactProfileImage",
    oAuth2Scopes: ["event/contacts:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ContactFile$inboundSchema),
