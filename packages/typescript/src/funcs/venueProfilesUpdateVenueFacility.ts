 ...
 */
export function venueProfilesUpdateVenueFacility(
  client: CventSDKCore,
  request: operations.UpdateVenueFacilityRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExistingVenueFacility,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateVenueFacilityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.existingVenueFacility, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExistingVenueFacility$inboundSchema),
