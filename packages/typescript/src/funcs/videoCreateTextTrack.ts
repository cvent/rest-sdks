 ...
 */
export function videoCreateTextTrack(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateTextTrackRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["video-text-track"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/videos/{videoId}/text-tracks")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ExistingVideoTextTrack$inboundSchema),
