 ...
 */
export function budgetAttachInvoiceToPayment(
  client: CventSDKCore,
  request: operations.AttachInvoiceToPaymentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.InvoiceFile,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AttachInvoiceToPaymentRequest$outboundSchema.parse(value),
  ...
  const context = {
    ...
    oAuth2Scopes: ["budget/payments:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InvoiceFile$inboundSchema),
