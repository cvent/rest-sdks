...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHousingEventInfoRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/housing-events/{housingEventId}")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["housing/housing-events:read"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.HousingEvent$inboundSchema),
