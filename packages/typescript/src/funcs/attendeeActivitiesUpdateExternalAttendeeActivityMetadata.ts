 ...
 */
export function attendeeActivitiesUpdateExternalAttendeeActivityMetadata(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateExternalAttendeeActivityMetadataRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON("body", payload["external-activity-metadata"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/attendees/activities/external/metadata/{id}")(
    pathParams,
  );
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/attendee-activities-metadata:write"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExternalActivityMetadata$inboundSchema),
