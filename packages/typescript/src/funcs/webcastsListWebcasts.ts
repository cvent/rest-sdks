 ...
 */
export function webcastsListWebcasts(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.ListWebcastsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListWebcastsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/webcasts")();
  ...
  const context = {
    ...
    oAuth2Scopes: ["event/webcasts:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListWebcastsResponse$inboundSchema, {
      key: "Result",
    }),
