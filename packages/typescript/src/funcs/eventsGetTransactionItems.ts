 ...
 */
export function eventsGetTransactionItems(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetTransactionItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTransactionItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/events/{id}/transactions/items")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetTransactionItemsResponse$inboundSchema, {
      key: "Result",
    }),
