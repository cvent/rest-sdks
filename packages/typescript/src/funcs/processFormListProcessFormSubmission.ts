 ...
 */
export function processFormListProcessFormSubmission(
  client: CventSDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListProcessFormSubmissionRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/process-forms/submissions")();
  ...
  const context = {
    ...
    operationID: "listProcessFormSubmission",
    oAuth2Scopes: ["event/process-form-submissions:read"],
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListProcessFormSubmissionResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      processFormListProcessFormSubmission(
        client,
        {
          ...request,
          token: nextCursor,
