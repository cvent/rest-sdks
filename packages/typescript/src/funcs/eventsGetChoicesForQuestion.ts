 ...
 */
export function eventsGetChoicesForQuestion(
  client: CventSDKCore,
  ...
  PageIterator<
    Result<
      operations.GetChoicesForQuestionResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetChoicesForQuestionRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    questionId: encodeSimple("questionId", payload.questionId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/event-questions/{questionId}/choices")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetChoicesForQuestionResponse$inboundSchema, {
      key: "Result",
    }),
