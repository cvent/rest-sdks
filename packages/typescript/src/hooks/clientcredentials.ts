  ...
  implements SDKInitHook, BeforeRequestHook, AfterErrorHook
{
  ...
  private async getCredentialsGlobal(
    ...
    const envTokenURL = env().CVENTSDK_TOKEN_URL ?? DEFAULT_TOKEN_URL;

    const additionalProperties: Record<string, string> = {};
    for (
      const [key, value] of Object.entries(
        out?.["OAuth2.clientCredentials"] ?? {},
      )
    ) {
      if (
        typeof value === "string"
    ...
    return {
      clientID: out?.["OAuth2.clientCredentials"]?.clientID
        ?? env().CVENTSDK_CLIENT_ID ?? "",
      clientSecret: out?.["OAuth2.clientCredentials"]?.clientSecret
        ?? env().CVENTSDK_CLIENT_SECRET ?? "",
      tokenURL:
        out?.["OAuth2.clientCredentials"]?.tokenURL !== DEFAULT_TOKEN_URL
          ? out?.["OAuth2.clientCredentials"]?.tokenURL
          : envTokenURL,
      scopes: out?.["OAuth2.clientCredentials"]?.scopes ?? env().CVENTSDK_SCOPES
        ?? undefined,
