# ~~DeprecatedAttendeeInvitedByJson~~

Method by which the attendee was invited to the event. Note: This field is deprecated. Previous documentation incorrectly listed support for this feature.

> :warning: **DEPRECATED**: This will be removed in a future release, please migrate away from it as soon as possible.

## Example Usage

```java
import com.cvent.models.components.DeprecatedAttendeeInvitedByJson;

DeprecatedAttendeeInvitedByJson value = DeprecatedAttendeeInvitedByJson.PLANNER_INVITED;
```


## Values

| Name                 | Value                |
| -------------------- | -------------------- |
| `PLANNER_INVITED`    | Planner-Invited      |
| `NO_INVITATION`      | No Invitation        |
| `GUEST_INVITED`      | Guest-Invited        |
| `ALTERNATE_CAMPAIGN` | Alternate Campaign   |
| `WEBLINK`            | Weblink              |
| `IMPORT`             | Import               |
| `HISTORICAL_IMPORT`  | Historical Import    |
| `API_INVITED`        | API Invited          |