/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.LazySingletonValue;
import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.core.type.TypeReference;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Boolean;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;

/**
 * AddressJson2
 *
 * <p>Address
 */
public class AddressJson2 {
    /**
     * The type of the address.
     */
    @JsonProperty("type")
    private AddressTypeJson type;

    /**
     * The street address of the user.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("streetAddress")
    private String streetAddress;

    /**
     * The locality/city of the user.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("locality")
    private String locality;

    /**
     * The region/state/province of the user.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("region")
    private String region;

    /**
     * Postal code (also known as zipcode) of the user.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("postalCode")
    private String postalCode;

    /**
     * The country of the user.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("country")
    private String country;

    /**
     * True indicates the address is primary.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("primary")
    private Boolean primary;

    @JsonCreator
    public AddressJson2(
            @JsonProperty("type") @Nonnull AddressTypeJson type,
            @JsonProperty("streetAddress") @Nullable String streetAddress,
            @JsonProperty("locality") @Nullable String locality,
            @JsonProperty("region") @Nullable String region,
            @JsonProperty("postalCode") @Nullable String postalCode,
            @JsonProperty("country") @Nullable String country,
            @JsonProperty("primary") @Nullable Boolean primary) {
        this.type = Optional.ofNullable(type).orElseThrow(() -> new IllegalArgumentException("type cannot be null"));
        this.streetAddress = streetAddress;
        this.locality = locality;
        this.region = region;
        this.postalCode = postalCode;
        this.country = country;
        this.primary = Optional.ofNullable(primary).orElse(Builder._SINGLETON_VALUE_Primary.value());
    }

    public AddressJson2(@Nonnull AddressTypeJson type) {
        this(type, null, null, null, null, null, null);
    }

    /**
     * The type of the address.
     */
    public AddressTypeJson type() {
        return this.type;
    }

    /**
     * The street address of the user.
     */
    public Optional<String> streetAddress() {
        return Optional.ofNullable(this.streetAddress);
    }

    /**
     * The locality/city of the user.
     */
    public Optional<String> locality() {
        return Optional.ofNullable(this.locality);
    }

    /**
     * The region/state/province of the user.
     */
    public Optional<String> region() {
        return Optional.ofNullable(this.region);
    }

    /**
     * Postal code (also known as zipcode) of the user.
     */
    public Optional<String> postalCode() {
        return Optional.ofNullable(this.postalCode);
    }

    /**
     * The country of the user.
     */
    public Optional<String> country() {
        return Optional.ofNullable(this.country);
    }

    /**
     * True indicates the address is primary.
     */
    public Optional<Boolean> primary() {
        return Optional.ofNullable(this.primary);
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The type of the address.
     */
    public AddressJson2 withType(@Nonnull AddressTypeJson type) {
        this.type = Utils.checkNotNull(type, "type");
        return this;
    }

    /**
     * The street address of the user.
     */
    public AddressJson2 withStreetAddress(@Nullable String streetAddress) {
        this.streetAddress = streetAddress;
        return this;
    }

    /**
     * The locality/city of the user.
     */
    public AddressJson2 withLocality(@Nullable String locality) {
        this.locality = locality;
        return this;
    }

    /**
     * The region/state/province of the user.
     */
    public AddressJson2 withRegion(@Nullable String region) {
        this.region = region;
        return this;
    }

    /**
     * Postal code (also known as zipcode) of the user.
     */
    public AddressJson2 withPostalCode(@Nullable String postalCode) {
        this.postalCode = postalCode;
        return this;
    }

    /**
     * The country of the user.
     */
    public AddressJson2 withCountry(@Nullable String country) {
        this.country = country;
        return this;
    }

    /**
     * True indicates the address is primary.
     */
    public AddressJson2 withPrimary(@Nullable Boolean primary) {
        this.primary = primary;
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        AddressJson2 other = (AddressJson2) o;
        return Utils.enhancedDeepEquals(this.type, other.type)
                && Utils.enhancedDeepEquals(this.streetAddress, other.streetAddress)
                && Utils.enhancedDeepEquals(this.locality, other.locality)
                && Utils.enhancedDeepEquals(this.region, other.region)
                && Utils.enhancedDeepEquals(this.postalCode, other.postalCode)
                && Utils.enhancedDeepEquals(this.country, other.country)
                && Utils.enhancedDeepEquals(this.primary, other.primary);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(type, streetAddress, locality, region, postalCode, country, primary);
    }

    @Override
    public String toString() {
        return Utils.toString(
                AddressJson2.class,
                "type",
                type,
                "streetAddress",
                streetAddress,
                "locality",
                locality,
                "region",
                region,
                "postalCode",
                postalCode,
                "country",
                country,
                "primary",
                primary);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private AddressTypeJson type;

        private String streetAddress;

        private String locality;

        private String region;

        private String postalCode;

        private String country;

        private Boolean primary;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The type of the address.
         */
        public Builder type(@Nonnull AddressTypeJson type) {
            this.type = Utils.checkNotNull(type, "type");
            return this;
        }

        /**
         * The street address of the user.
         */
        public Builder streetAddress(@Nullable String streetAddress) {
            this.streetAddress = streetAddress;
            return this;
        }

        /**
         * The locality/city of the user.
         */
        public Builder locality(@Nullable String locality) {
            this.locality = locality;
            return this;
        }

        /**
         * The region/state/province of the user.
         */
        public Builder region(@Nullable String region) {
            this.region = region;
            return this;
        }

        /**
         * Postal code (also known as zipcode) of the user.
         */
        public Builder postalCode(@Nullable String postalCode) {
            this.postalCode = postalCode;
            return this;
        }

        /**
         * The country of the user.
         */
        public Builder country(@Nullable String country) {
            this.country = country;
            return this;
        }

        /**
         * True indicates the address is primary.
         */
        public Builder primary(@Nullable Boolean primary) {
            this.primary = primary;
            return this;
        }

        public AddressJson2 build() {
            return new AddressJson2(type, streetAddress, locality, region, postalCode, country, primary);
        }

        private static final LazySingletonValue<Boolean> _SINGLETON_VALUE_Primary =
                new LazySingletonValue<>("primary", "false", new TypeReference<Boolean>() {});
    }
}
