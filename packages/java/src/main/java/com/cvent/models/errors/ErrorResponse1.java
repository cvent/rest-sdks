...
public class ErrorResponse1 extends CventSDKError {
    ...
    /**
     * Parse a response into an instance of ErrorResponse1. If deserialization of the response body fails,
     * the resulting ErrorResponse1 instance will have a null data() value and a non-null deserializationException().
     */
    public static ErrorResponse1 from(HttpResponse<InputStream> response) {
        try {
            byte[] bytes = Utils.extractByteArrayFromBody(response);
            Data data = Utils.mapper().readValue(bytes, Data.class);
            return new ErrorResponse1(response.statusCode(), bytes, response, data, null);
        } catch (Exception e) {
            return new ErrorResponse1(response.statusCode(), null, response, null, e);
        }
    }

    /**
     * Parse a response into an instance of ErrorResponse1 asynchronously. If deserialization of the response body fails,
     * the resulting ErrorResponse1 instance will have a null data() value and a non-null deserializationException().
     */
    public static CompletableFuture<ErrorResponse1> fromAsync(HttpResponse<Blob> response) {
        return response.body().toByteArray().handle((bytes, err) -> {
            ...
            try {
                ...
                return new ErrorResponse1(response.statusCode(), bytes, response, null, e);
            }
        });
    }
