/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;

/**
 * VideoClip
 *
 * <p>Video clip details.
 */
public class VideoClip {
    /**
     * The timecode to start the clip in HH:MM:SS:FF format
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("start")
    private String start;

    /**
     * The timecode to end the clip in HH:MM:SS:FF format.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("end")
    private String end;

    @JsonCreator
    public VideoClip(@JsonProperty("start") @Nullable String start, @JsonProperty("end") @Nullable String end) {
        this.start = start;
        this.end = end;
    }

    public VideoClip() {
        this(null, null);
    }

    /**
     * The timecode to start the clip in HH:MM:SS:FF format
     */
    public Optional<String> start() {
        return Optional.ofNullable(this.start);
    }

    /**
     * The timecode to end the clip in HH:MM:SS:FF format.
     */
    public Optional<String> end() {
        return Optional.ofNullable(this.end);
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The timecode to start the clip in HH:MM:SS:FF format
     */
    public VideoClip withStart(@Nullable String start) {
        this.start = start;
        return this;
    }

    /**
     * The timecode to end the clip in HH:MM:SS:FF format.
     */
    public VideoClip withEnd(@Nullable String end) {
        this.end = end;
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        VideoClip other = (VideoClip) o;
        return Utils.enhancedDeepEquals(this.start, other.start) && Utils.enhancedDeepEquals(this.end, other.end);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(start, end);
    }

    @Override
    public String toString() {
        return Utils.toString(VideoClip.class, "start", start, "end", end);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private String start;

        private String end;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The timecode to start the clip in HH:MM:SS:FF format
         */
        public Builder start(@Nullable String start) {
            this.start = start;
            return this;
        }

        /**
         * The timecode to end the clip in HH:MM:SS:FF format.
         */
        public Builder end(@Nullable String end) {
            this.end = end;
            return this;
        }

        public VideoClip build() {
            return new VideoClip(start, end);
        }
    }
}
