/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;

/**
 * Group
 *
 * <p>Generic sample name. Not a real model used by any Cvent service.
 */
public class Group {
    /**
     * The unique ID of the SCIM group.
     */
    @JsonProperty("value")
    private String value;

    /**
     * The name of the user role.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("display")
    private String display;

    @JsonCreator
    public Group(@JsonProperty("value") @Nonnull String value, @JsonProperty("display") @Nullable String display) {
        this.value = Optional.ofNullable(value).orElseThrow(() -> new IllegalArgumentException("value cannot be null"));
        this.display = display;
    }

    public Group(@Nonnull String value) {
        this(value, null);
    }

    /**
     * The unique ID of the SCIM group.
     */
    public String value() {
        return this.value;
    }

    /**
     * The name of the user role.
     */
    public Optional<String> display() {
        return Optional.ofNullable(this.display);
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The unique ID of the SCIM group.
     */
    public Group withValue(@Nonnull String value) {
        this.value = Utils.checkNotNull(value, "value");
        return this;
    }

    /**
     * The name of the user role.
     */
    public Group withDisplay(@Nullable String display) {
        this.display = display;
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        Group other = (Group) o;
        return Utils.enhancedDeepEquals(this.value, other.value) && Utils.enhancedDeepEquals(this.display, other.display);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(value, display);
    }

    @Override
    public String toString() {
        return Utils.toString(Group.class, "value", value, "display", display);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private String value;

        private String display;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The unique ID of the SCIM group.
         */
        public Builder value(@Nonnull String value) {
            this.value = Utils.checkNotNull(value, "value");
            return this;
        }

        /**
         * The name of the user role.
         */
        public Builder display(@Nullable String display) {
            this.display = display;
            return this;
        }

        public Group build() {
            return new Group(value, display);
        }
    }
}
