/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;

/**
 * ContactMergeField
 *
 * <p>A pair of a contact field name and a contact id.
 */
public class ContactMergeField {
    /**
     * The name of a contact field.
     */
    @JsonProperty("fieldName")
    private String fieldName;

    /**
     * ID of the contact.
     */
    @JsonProperty("contactId")
    private String contactId;

    @JsonCreator
    public ContactMergeField(
            @JsonProperty("fieldName") @Nonnull String fieldName,
            @JsonProperty("contactId") @Nonnull String contactId) {
        this.fieldName = Optional.ofNullable(fieldName)
                .orElseThrow(() -> new IllegalArgumentException("fieldName cannot be null"));
        this.contactId = Optional.ofNullable(contactId)
                .orElseThrow(() -> new IllegalArgumentException("contactId cannot be null"));
    }

    /**
     * The name of a contact field.
     */
    public String fieldName() {
        return this.fieldName;
    }

    /**
     * ID of the contact.
     */
    public String contactId() {
        return this.contactId;
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The name of a contact field.
     */
    public ContactMergeField withFieldName(@Nonnull String fieldName) {
        this.fieldName = Utils.checkNotNull(fieldName, "fieldName");
        return this;
    }

    /**
     * ID of the contact.
     */
    public ContactMergeField withContactId(@Nonnull String contactId) {
        this.contactId = Utils.checkNotNull(contactId, "contactId");
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        ContactMergeField other = (ContactMergeField) o;
        return Utils.enhancedDeepEquals(this.fieldName, other.fieldName)
                && Utils.enhancedDeepEquals(this.contactId, other.contactId);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(fieldName, contactId);
    }

    @Override
    public String toString() {
        return Utils.toString(ContactMergeField.class, "fieldName", fieldName, "contactId", contactId);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private String fieldName;

        private String contactId;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The name of a contact field.
         */
        public Builder fieldName(@Nonnull String fieldName) {
            this.fieldName = Utils.checkNotNull(fieldName, "fieldName");
            return this;
        }

        /**
         * ID of the contact.
         */
        public Builder contactId(@Nonnull String contactId) {
            this.contactId = Utils.checkNotNull(contactId, "contactId");
            return this;
        }

        public ContactMergeField build() {
            return new ContactMergeField(fieldName, contactId);
        }
    }
}
