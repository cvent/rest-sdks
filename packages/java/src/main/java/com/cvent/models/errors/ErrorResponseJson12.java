/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.errors;

import com.cvent.models.components.ErrorScimTypeJson;
import com.cvent.utils.Blob;
import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.core.type.TypeReference;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.io.InputStream;
import java.lang.Deprecated;
import java.lang.Exception;
import java.lang.Long;
import java.lang.Override;
import java.lang.String;
import java.lang.SuppressWarnings;
import java.lang.Throwable;
import java.net.http.HttpResponse;
import java.util.List;
import java.util.Optional;
import java.util.concurrent.CompletableFuture;

@SuppressWarnings("serial")
public class ErrorResponseJson12 extends CventSDKError {

    @Nullable
    private final Data data;

    @Nullable
    private final Throwable deserializationException;

    public ErrorResponseJson12(
            int code,
            byte[] body,
            HttpResponse<?> rawResponse,
            @Nullable Data data,
            @Nullable Throwable deserializationException) {
        super("API error occurred", code, body, rawResponse, null);
        this.data = data;
        this.deserializationException = deserializationException;
    }

    /**
     * Parse a response into an instance of ErrorResponseJson12. If deserialization of the response body fails,
     * the resulting ErrorResponseJson12 instance will have a null data() value and a non-null deserializationException().
     */
    public static ErrorResponseJson12 from(HttpResponse<InputStream> response) {
        try {
            byte[] bytes = Utils.extractByteArrayFromBody(response);
            Data data = Utils.mapper().readValue(bytes, Data.class);
            return new ErrorResponseJson12(response.statusCode(), bytes, response, data, null);
        } catch (Exception e) {
            return new ErrorResponseJson12(response.statusCode(), null, response, null, e);
        }
    }

    /**
     * Parse a response into an instance of ErrorResponseJson12 asynchronously. If deserialization of the response body fails,
     * the resulting ErrorResponseJson12 instance will have a null data() value and a non-null deserializationException().
     */
    public static CompletableFuture<ErrorResponseJson12> fromAsync(HttpResponse<Blob> response) {
        return response.body().toByteArray().handle((bytes, err) -> {
            // if a body read error occurs, we want to transform the exception
            if (err != null) {
                throw new AsyncAPIException(
                        "Error reading response body: " + err.getMessage(), response.statusCode(), null, response, err);
            }

            try {
                return new ErrorResponseJson12(
                        response.statusCode(),
                        bytes,
                        response,
                        Utils.mapper().readValue(bytes, new TypeReference<Data>() {}),
                        null);
            } catch (Exception e) {
                return new ErrorResponseJson12(response.statusCode(), bytes, response, null, e);
            }
        });
    }

    /**
     * The collection of error schemas.
     */
    @Deprecated
    public Optional<List<String>> schemas() {
        return data().flatMap(Data::schemas);
    }

    /**
     * Status code for error.
     */
    @Deprecated
    public Optional<Long> status() {
        return data().map(Data::status);
    }

    /**
     * Details of the error.
     */
    @Deprecated
    public Optional<String> detail() {
        return data().map(Data::detail);
    }

    /**
     * This is used to denote the scim type of the error.
     */
    @Deprecated
    public Optional<ErrorScimTypeJson> scimType() {
        return data().flatMap(Data::scimType);
    }

    public Optional<Data> data() {
        return Optional.ofNullable(data);
    }

    /**
     * Returns the exception if an error occurs while deserializing the response body.
     */
    public Optional<Throwable> deserializationException() {
        return Optional.ofNullable(deserializationException);
    }
    /**
     * Data
     *
     * <p>The error response.
     */
    public static class Data {
        /**
         * The collection of error schemas.
         */
        @JsonInclude(Include.NON_ABSENT)
        @JsonProperty("schemas")
        private List<String> schemas;

        /**
         * Status code for error.
         */
        @JsonProperty("status")
        private long status;

        /**
         * Details of the error.
         */
        @JsonProperty("detail")
        private String detail;

        /**
         * This is used to denote the scim type of the error.
         */
        @JsonInclude(Include.NON_ABSENT)
        @JsonProperty("scimType")
        private ErrorScimTypeJson scimType;

        @JsonCreator
        public Data(
                @JsonProperty("schemas") @Nullable List<String> schemas,
                @JsonProperty("status") long status,
                @JsonProperty("detail") @Nonnull String detail,
                @JsonProperty("scimType") @Nullable ErrorScimTypeJson scimType) {
            this.schemas = schemas;
            this.status = status;
            this.detail = Optional.ofNullable(detail)
                    .orElseThrow(() -> new IllegalArgumentException("detail cannot be null"));
            this.scimType = scimType;
        }

        public Data(long status, @Nonnull String detail) {
            this(null, status, detail, null);
        }

        /**
         * The collection of error schemas.
         */
        public Optional<List<String>> schemas() {
            return Optional.ofNullable(this.schemas);
        }

        /**
         * Status code for error.
         */
        public long status() {
            return this.status;
        }

        /**
         * Details of the error.
         */
        public String detail() {
            return this.detail;
        }

        /**
         * This is used to denote the scim type of the error.
         */
        public Optional<ErrorScimTypeJson> scimType() {
            return Optional.ofNullable(this.scimType);
        }

        public static Builder builder() {
            return new Builder();
        }

        /**
         * The collection of error schemas.
         */
        public Data withSchemas(@Nullable List<String> schemas) {
            this.schemas = schemas;
            return this;
        }

        /**
         * Status code for error.
         */
        public Data withStatus(long status) {
            this.status = status;
            return this;
        }

        /**
         * Details of the error.
         */
        public Data withDetail(@Nonnull String detail) {
            this.detail = Utils.checkNotNull(detail, "detail");
            return this;
        }

        /**
         * This is used to denote the scim type of the error.
         */
        public Data withScimType(@Nullable ErrorScimTypeJson scimType) {
            this.scimType = scimType;
            return this;
        }

        @Override
        public boolean equals(java.lang.Object o) {
            if (this == o) {
                return true;
            }
            if (o == null || getClass() != o.getClass()) {
                return false;
            }
            Data other = (Data) o;
            return Utils.enhancedDeepEquals(this.schemas, other.schemas)
                    && Utils.enhancedDeepEquals(this.status, other.status)
                    && Utils.enhancedDeepEquals(this.detail, other.detail)
                    && Utils.enhancedDeepEquals(this.scimType, other.scimType);
        }

        @Override
        public int hashCode() {
            return Utils.enhancedHash(schemas, status, detail, scimType);
        }

        @Override
        public String toString() {
            return Utils.toString(
                    Data.class, "schemas", schemas, "status", status, "detail", detail, "scimType", scimType);
        }

        @SuppressWarnings("UnusedReturnValue")
        public static final class Builder {

            private List<String> schemas;

            private long status;

            private String detail;

            private ErrorScimTypeJson scimType;

            private Builder() {
                // force use of static builder() method
            }

            /**
             * The collection of error schemas.
             */
            public Builder schemas(@Nullable List<String> schemas) {
                this.schemas = schemas;
                return this;
            }

            /**
             * Status code for error.
             */
            public Builder status(long status) {
                this.status = status;
                return this;
            }

            /**
             * Details of the error.
             */
            public Builder detail(@Nonnull String detail) {
                this.detail = Utils.checkNotNull(detail, "detail");
                return this;
            }

            /**
             * This is used to denote the scim type of the error.
             */
            public Builder scimType(@Nullable ErrorScimTypeJson scimType) {
                this.scimType = scimType;
                return this;
            }

            public Data build() {
                return new Data(schemas, status, detail, scimType);
            }
        }
    }
}
