/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Optional;

/**
 * AttendeeAnswer1
 *
 * <p>Answer for the question answered by attendee.
 */
public class AttendeeAnswer1 {
    /**
     * The reference to the question entity. Contains only the ID of the question entity.
     */
    @JsonProperty("question")
    private AttendeeAnswer1Question question;

    /**
     * The set of answers to a question.
     */
    @JsonProperty("value")
    private List<String> value;

    @JsonCreator
    public AttendeeAnswer1(
            @JsonProperty("question") @Nonnull AttendeeAnswer1Question question,
            @JsonProperty("value") @Nonnull List<String> value) {
        this.question = Optional.ofNullable(question)
                .orElseThrow(() -> new IllegalArgumentException("question cannot be null"));
        this.value = Optional.ofNullable(value).orElseThrow(() -> new IllegalArgumentException("value cannot be null"));
    }

    /**
     * The reference to the question entity. Contains only the ID of the question entity.
     */
    public AttendeeAnswer1Question question() {
        return this.question;
    }

    /**
     * The set of answers to a question.
     */
    public List<String> value() {
        return this.value;
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The reference to the question entity. Contains only the ID of the question entity.
     */
    public AttendeeAnswer1 withQuestion(@Nonnull AttendeeAnswer1Question question) {
        this.question = Utils.checkNotNull(question, "question");
        return this;
    }

    /**
     * The set of answers to a question.
     */
    public AttendeeAnswer1 withValue(@Nonnull List<String> value) {
        this.value = Utils.checkNotNull(value, "value");
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        AttendeeAnswer1 other = (AttendeeAnswer1) o;
        return Utils.enhancedDeepEquals(this.question, other.question) && Utils.enhancedDeepEquals(this.value, other.value);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(question, value);
    }

    @Override
    public String toString() {
        return Utils.toString(AttendeeAnswer1.class, "question", question, "value", value);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private AttendeeAnswer1Question question;

        private List<String> value;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The reference to the question entity. Contains only the ID of the question entity.
         */
        public Builder question(@Nonnull AttendeeAnswer1Question question) {
            this.question = Utils.checkNotNull(question, "question");
            return this;
        }

        /**
         * The set of answers to a question.
         */
        public Builder value(@Nonnull List<String> value) {
            this.value = Utils.checkNotNull(value, "value");
            return this;
        }

        public AttendeeAnswer1 build() {
            return new AttendeeAnswer1(question, value);
        }
    }
}
