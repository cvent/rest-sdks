/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;

/**
 * SurveyContactId
 *
 * <p>A pair of survey and contact id.
 */
public class SurveyContactId {
    /**
     * ID of the survey
     */
    @JsonProperty("surveyId")
    private String surveyId;

    /**
     * ID of the contact
     */
    @JsonProperty("contactId")
    private String contactId;

    @JsonCreator
    public SurveyContactId(
            @JsonProperty("surveyId") @Nonnull String surveyId, @JsonProperty("contactId") @Nonnull String contactId) {
        this.surveyId = Optional.ofNullable(surveyId)
                .orElseThrow(() -> new IllegalArgumentException("surveyId cannot be null"));
        this.contactId = Optional.ofNullable(contactId)
                .orElseThrow(() -> new IllegalArgumentException("contactId cannot be null"));
    }

    /**
     * ID of the survey
     */
    public String surveyId() {
        return this.surveyId;
    }

    /**
     * ID of the contact
     */
    public String contactId() {
        return this.contactId;
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * ID of the survey
     */
    public SurveyContactId withSurveyId(@Nonnull String surveyId) {
        this.surveyId = Utils.checkNotNull(surveyId, "surveyId");
        return this;
    }

    /**
     * ID of the contact
     */
    public SurveyContactId withContactId(@Nonnull String contactId) {
        this.contactId = Utils.checkNotNull(contactId, "contactId");
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        SurveyContactId other = (SurveyContactId) o;
        return Utils.enhancedDeepEquals(this.surveyId, other.surveyId)
                && Utils.enhancedDeepEquals(this.contactId, other.contactId);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(surveyId, contactId);
    }

    @Override
    public String toString() {
        return Utils.toString(SurveyContactId.class, "surveyId", surveyId, "contactId", contactId);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private String surveyId;

        private String contactId;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * ID of the survey
         */
        public Builder surveyId(@Nonnull String surveyId) {
            this.surveyId = Utils.checkNotNull(surveyId, "surveyId");
            return this;
        }

        /**
         * ID of the contact
         */
        public Builder contactId(@Nonnull String contactId) {
            this.contactId = Utils.checkNotNull(contactId, "contactId");
            return this;
        }

        public SurveyContactId build() {
            return new SurveyContactId(surveyId, contactId);
        }
    }
}
