/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Optional;

/**
 * AttendeeType
 *
 * <p>Attendee type details.
 */
public class AttendeeType {
    /**
     * The unique ID of the attendee type.
     */
    @JsonProperty("id")
    private long id;

    /**
     * The name of the attendee type.
     */
    @JsonProperty("name")
    private String name;

    /**
     * A list of attendee type codes.
     */
    @JsonProperty("codes")
    private List<String> codes;

    @JsonCreator
    public AttendeeType(
            @JsonProperty("id") long id,
            @JsonProperty("name") @Nonnull String name,
            @JsonProperty("codes") @Nonnull List<String> codes) {
        this.id = id;
        this.name = Optional.ofNullable(name).orElseThrow(() -> new IllegalArgumentException("name cannot be null"));
        this.codes = Optional.ofNullable(codes).orElseThrow(() -> new IllegalArgumentException("codes cannot be null"));
    }

    /**
     * The unique ID of the attendee type.
     */
    public long id() {
        return this.id;
    }

    /**
     * The name of the attendee type.
     */
    public String name() {
        return this.name;
    }

    /**
     * A list of attendee type codes.
     */
    public List<String> codes() {
        return this.codes;
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The unique ID of the attendee type.
     */
    public AttendeeType withId(long id) {
        this.id = id;
        return this;
    }

    /**
     * The name of the attendee type.
     */
    public AttendeeType withName(@Nonnull String name) {
        this.name = Utils.checkNotNull(name, "name");
        return this;
    }

    /**
     * A list of attendee type codes.
     */
    public AttendeeType withCodes(@Nonnull List<String> codes) {
        this.codes = Utils.checkNotNull(codes, "codes");
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        AttendeeType other = (AttendeeType) o;
        return Utils.enhancedDeepEquals(this.id, other.id)
                && Utils.enhancedDeepEquals(this.name, other.name)
                && Utils.enhancedDeepEquals(this.codes, other.codes);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(id, name, codes);
    }

    @Override
    public String toString() {
        return Utils.toString(AttendeeType.class, "id", id, "name", name, "codes", codes);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private long id;

        private String name;

        private List<String> codes;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The unique ID of the attendee type.
         */
        public Builder id(long id) {
            this.id = id;
            return this;
        }

        /**
         * The name of the attendee type.
         */
        public Builder name(@Nonnull String name) {
            this.name = Utils.checkNotNull(name, "name");
            return this;
        }

        /**
         * A list of attendee type codes.
         */
        public Builder codes(@Nonnull List<String> codes) {
            this.codes = Utils.checkNotNull(codes, "codes");
            return this;
        }

        public AttendeeType build() {
            return new AttendeeType(id, name, codes);
        }
    }
}
