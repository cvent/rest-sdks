/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.cvent.models.components;

import com.cvent.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Optional;

/**
 * AlternateAnswer
 *
 * <p>Represents the answer(s) to an alternate travel question submitted by an attendee.
 */
public class AlternateAnswer {
    /**
     * The set of answers to a single alternate travel question.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("value")
    private List<String> value;

    @JsonCreator
    public AlternateAnswer(@JsonProperty("value") @Nullable List<String> value) {
        this.value = value;
    }

    public AlternateAnswer() {
        this(null);
    }

    /**
     * The set of answers to a single alternate travel question.
     */
    public Optional<List<String>> value() {
        return Optional.ofNullable(this.value);
    }

    public static Builder builder() {
        return new Builder();
    }

    /**
     * The set of answers to a single alternate travel question.
     */
    public AlternateAnswer withValue(@Nullable List<String> value) {
        this.value = value;
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        AlternateAnswer other = (AlternateAnswer) o;
        return Utils.enhancedDeepEquals(this.value, other.value);
    }

    @Override
    public int hashCode() {
        return Utils.enhancedHash(value);
    }

    @Override
    public String toString() {
        return Utils.toString(AlternateAnswer.class, "value", value);
    }

    @SuppressWarnings("UnusedReturnValue")
    public static final class Builder {

        private List<String> value;

        private Builder() {
            // force use of static builder() method
        }

        /**
         * The set of answers to a single alternate travel question.
         */
        public Builder value(@Nullable List<String> value) {
            this.value = value;
            return this;
        }

        public AlternateAnswer build() {
            return new AlternateAnswer(value);
        }
    }
}
