 ...
 * <p># Getting Started
 * 
 * <p>This SDK is generated by [Speakeasy](https://speakeasy.com). For usage instructions and code
 * examples,
 ...
 * 
 * <p>The Cvent REST API uses [OAuth2](https://oauth.net/2/) for authorization. The client credentials
 * flow is supported.
 ...
 * 
 * <p>The `hostname` will depend on the region that your Cvent account is hosted in:
 * 
 * <p>| Region        | Hostname                           |
 * |:--------------|:-----------------------------------|
 ...
 * 
 * <p>- **Daily calls** define how many requests you can make in a 24-hour period. Quota resets at 12
 * midnight (+0 GMT).
 ...
 * 
 * <p>Check your tier via [Get Current Usage Tier](#operation/getUsageTier). These limits may change as
 * the platform evolves.
 * 
 * <p>### Handling Rate Limits
 * 
 * <p>When you exceed rate limits, the API returns `429 Too Many Requests`. Implement exponential backoff
 ...
 * `filter='field' comparisonType 'value'`. The value can be enclosed with single quotes (') or double
 * quotes (").
 * 
 * <p>```bash
 ...
 * ```
 * 
 * <p>To pass a single quote in the filter's value, use double quotes around the string.
 * 
 * <p>```bash
 * GET {hostName}/{version}/contacts?filter=lastName eq "O'Keenan"
 * ```
 * 
 * <p>To pass a double quote, use double quotes around the string and escape each inner quote with `\`.
 * 
 * <p>```bash
 ...
 * <p>## Versioning
 * 
 * <p>We track both backward-compatible and backward-incompatible changes.
 * 
 * <p>**Backward compatible:** Adding new resources, optional parameters, or attributes; changing format
 * (not type) of IDs; increasing string field lengths.
 * 
 * <p>**Backward incompatible:** Adding required parameters; deleting resources or attributes; changing
 * data types. When breaking changes occur, a new API version is released while the old version remains
 * available temporarily.
 * 
 * <p>For documentation and guides, visit [developers.cvent.com](https://developers.cvent.com).
 * 
